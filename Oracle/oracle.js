/* ============================================================
   ...
   Built so far:
     ...
     L2         → How many players?  → suggested games
   ...
   player-count set). Digital-only titles use digitalOnly().
   Difficulty / duration questions come later.
   ============================================================ */
...
// ============================================================
const GAMES = [
    // Seven Wonders — designed for 2 or 4 players.
    ...
    game('Futory Cards Unity', 'futorycardsunity', 'Futory', 2, 4),
    game('Futory Cards Duality', 'futorycardsduality', 'Futory', 2, 4),

    // Casino Camino
    game('American Playing Cards', 'americanplayingcards', 'Casino Camino', 2, 6),
    game('Fortuna', 'fortuna', 'Casino Camino', 2, 8),
    game('Ricochet', 'ricochet', 'Casino Camino', 2, 8),

    // Believe
    game('Believe', 'believe', 'Believe', 2, 8),
    game('Believe Objects', 'believeobjects', 'Believe', 2, 8),

    // Detective Noname — solo, screen only
    digitalOnly('Detective Noname and the Silent Circle', 'noname', 'Detective Noname', 1, 1),

    // Screen-born solo games — no physical edition
    digitalOnly('Beat Race', 'beatrace', '', 1, 1),
    digitalOnly('Crosslink', 'crosslink', '', 1, 1),

    // Elements
    game('Elements', 'elements', 'Elements', 2, 6),

    // Nectar (brand to be confirmed)
    game('Nectar', 'nectar', '', 2, 6),
];
...
// physical: which of the 2/4 tables you can buy in a box (none by default).
function sw(name, slug, physical = []) {
    return { name, slug, brand: 'Seven Wonders', counts: [2, 4], physical };
}

// In print at every size it was designed for.
function game(name, slug, brand, min, max) {
    const counts = range(min, max);
    return { name, slug, brand, counts, physical: counts };
}

// A title that exists only on a screen — it never reaches the table.
function digitalOnly(name, slug, brand, min, max) {
    return { name, slug, brand, counts: range(min, max), physical: [] };
}
...
class Oracle {
    ...
    makeResultFloor(n, color, mode) {
        return { el: this.buildResultLevel(n, color, mode), node: null, kind: 'leaf', chosenIndex: null, accent: color };
    }
    ...
    buildPlayersLevel(node, floorIndex) {
        ...
        playerCounts(node.mode).forEach((n, i) => {
            ...
            valve.addEventListener('click', () =>
                this.commit(floorIndex, valve, i, node.color, () => this.makeResultFloor(n, node.color, node.mode))
            );
            valves.appendChild(valve);
        });
        return level;
    ...
    buildResultLevel(n, color, mode) {
        const level = document.createElement('section');
        level.className = 'level level-result';
        level.style.setProperty('--vc', color || 'var(--champagne)');

        const games = gamesFor(n, mode);
        const label = n === 1 ? 'Solo' : `${n} Players`;
        const count = games.length;
        const where = mode === 'digital' ? 'on screen' : 'for your table';
